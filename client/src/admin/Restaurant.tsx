...
<select
  ...
  onChange={(e) =>
    setInput({ ...input, deliveryTime: Number(e.target.value) })
  }
  className="border border-gray-300 rounded-md w-full p-2 text-sm focus:outline-none focus:ring-2 focus:ring-purple"
>
  <option value={0}>Select delivery time</option>
  <option value={15}>15–30 mins</option>
  <option value={30}>30–45 mins</option>
  <option value={45}>45–60 mins</option>
  <option value={60}>1–1.5 hours</option>
</select>
{errors && (
  <span className="text-xs text-red-600 font-medium">
    {errors.deliveryTime}
